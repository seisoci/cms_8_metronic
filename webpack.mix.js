...
// Metronic css backend (single page use)
(glob.sync('resources/backend/css/**/!(_)*.css') || []).forEach(file => {
    file = file.replace(/[\\\/]+/g, '/');
    mix.css(file, file.replace('resources/backend/css', 'public/css/backend').replace(/\.css$/, '.css'));
});

// Metronic sass backend (single page use)
...
// Metronic js backend (single page use)
